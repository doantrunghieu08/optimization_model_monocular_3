...
from src.pipelines.preprocess.calib import resolve_selected_intrinsics
...
def get_video_map(config: dict) -> dict[str, str]:
    ...
    mp4_files = sorted(
        glob(str(search_dir / "*.mp4")),
        key=lambda x: int(re.search(r"(\d+)\.mp4$", x).group(1)) if re.search(r"(\d+)\.mp4$", x) else float("inf"),
    )
    if len(mp4_files) >= 2:
        video_map["camera1"] = mp4_files[0]
        video_map["camera2"] = mp4_files[1]
        print(f"[Visualization] Video map fallback from {search_dir}: {video_map}")
        return video_map

    raise ValueError("Visualization requires camera1_video/camera2_video or at least 2 mp4 files in video_search_dir")
...
def _resolve_image_dir(config: dict, camera_name: str) -> Path:
    paths = config.get("paths", {})
    inputs = config.get("inputs", {})
    cam_idx = "1" if camera_name == "camera1" else "2"
    cam_video = Path(inputs.get(f"camera{cam_idx}_video", f"video_{cam_idx}.mp4"))
    cam_video_basename = cam_video.stem
    preprocess_dir = paths.get("preprocess_output_dir", "optimization_model_monocular/output/preprocess_results")
    vis_dir = paths.get("visualization_output_dir", "optimization_model_monocular/output")
    candidates = [
        Path(f"{preprocess_dir}/images_{cam_video_basename}"),
        Path(f"{preprocess_dir}/images_video_{cam_idx}"),
        Path(f"{vis_dir}/image_video_{cam_idx}"),
        Path(f"{vis_dir}/images_video_{cam_idx}"),
        Path(f"{vis_dir}/images_cam{cam_idx}"),
    ]
    for c in candidates:
        if c.exists():
            return c
...
def _build_priority_metric_maps(paths: dict, metric_name: str = "PA-MPJPE", priority_suffix: str = "priority1") -> Dict[str, Dict[str, Dict[int, float]]]:
    ...
    eval_dir = Path(paths.get("evaluation_output_dir", "optimization_model_monocular/output/evaluation_results"))
...
def _create_project2d_animation_for_specs(camera_name: str, config: dict, paths: dict, output_video: Path, target_fps: int, module_specs: list[dict], dpi: int = 100) -> None:
    if not module_specs:
        ...
        return
    image_dir = _resolve_image_dir(config, camera_name)
...
"""## 13. Điều phối: định nghĩa run_pipeline(...)
