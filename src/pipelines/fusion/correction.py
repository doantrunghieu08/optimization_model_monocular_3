...
from src.pipelines.fusion.config import LIMB_CHAINS
from src.pipelines.fusion.config import MIN_LIMB_SOURCE_BELIEF
...
def _limb_mean_belief(belief, joints):
    values = [float(belief.get(name, 0.0)) for name in joints]
    return float(np.mean(values)) if values else 0.0


def _any_visible(vis, joints):
    return any(vis.get(name, True) for name in joints)


def _all_occluded(vis, joints):
    return bool(joints) and all(not vis.get(name, True) for name in joints)


def _max_bone_angle_deg(original_pose, candidate_pose, bones):
    max_deg = 0.0
    compared = False
    for child, parent in bones:
        if child not in original_pose or parent not in original_pose:
            continue
        if child not in candidate_pose or parent not in candidate_pose:
            continue
        original = _unit_vector(as_xyz(original_pose[child]) - as_xyz(original_pose[parent]))
        candidate = _unit_vector(as_xyz(candidate_pose[child]) - as_xyz(candidate_pose[parent]))
        if original is None or candidate is None:
            continue
        cosine = float(np.clip(np.dot(original, candidate), -1.0, 1.0))
        max_deg = max(max_deg, float(np.degrees(np.arccos(cosine))))
        compared = True
    return max_deg if compared else 0.0


def select_limb_winner_sets(h1, h2, vis1, vis2, delta, min_source_belief=MIN_LIMB_SOURCE_BELIEF):
    k1_set = set()
    k2_set = set()
    decisions = []
    for limb_name, spec in LIMB_CHAINS.items():
        joints = [name for name in spec["joints"] if name not in NON_REPLACEABLE_ANCHORS]
        mean1 = _limb_mean_belief(h1, joints)
        mean2 = _limb_mean_belief(h2, joints)
        winner = None
        reason = "insufficient_delta"
        min_slave_belief = max(min_source_belief, 0.15)
        if _all_occluded(vis2, joints) and _any_visible(vis1, joints) and mean1 >= min_source_belief:
            winner, reason = "camera1", "occlusion"
        elif _all_occluded(vis1, joints) and _any_visible(vis2, joints) and mean2 >= min_slave_belief and mean2 >= mean1:
            winner, reason = "camera2", "occlusion"
        elif mean1 > mean2 + delta and mean1 >= min_source_belief and _any_visible(vis1, joints):
            winner, reason = "camera1", "belief"
        elif mean2 > mean1 + delta and mean2 >= min_slave_belief and _any_visible(vis2, joints):
            winner, reason = "camera2", "belief"
        if winner == "camera1":
            k1_set.update(joints)
        elif winner == "camera2":
            k2_set.update(joints)
        decisions.append({
            "limb": limb_name,
            "winner": winner,
            "reason": reason if winner else "keep",
            "belief_camera1": mean1,
            "belief_camera2": mean2,
            "joints": joints,
        })
    return k1_set, k2_set, decisions


def apply_limb_winner_corrections(
    cam1,
    cam2,
    k1_set,
    k2_set,
    ...
    cam1_corr = {name: as_xyz(point) for name, point in cam1.items()}
    cam2_corr = {name: as_xyz(point) for name, point in cam2.items()}
    applied_k1 = set()
    applied_k2 = set()
    angle_limit = float(max_bone_angle_deg)
    decision_by_limb = {item["limb"]: item for item in (decisions or [])}

    def _map_point(point, source_pose, target_pose, transform):
        if root_relative:
            return apply_root_relative(point, source_pose, target_pose, transform)
        return apply_similarity(point, transform)

    for limb_name, spec in LIMB_CHAINS.items():
        joints = [name for name in spec["joints"] if name in k1_set or name in k2_set]
        if not joints:
            continue
        source_is_cam1 = any(name in k1_set for name in joints)
        source, target = (cam1, cam2_corr) if source_is_cam1 else (cam2, cam1_corr)
        transform = t12 if source_is_cam1 else t21
        candidate = dict(target)
        for name in joints:
            if name not in source or name not in target:
                continue
            candidate[name] = _map_point(source[name], source, target, transform)
        angle_deg = _max_bone_angle_deg(target, candidate, spec["bones"])
        accepted = angle_deg <= angle_limit
        if limb_name in decision_by_limb:
            decision_by_limb[limb_name]["angle_deg"] = angle_deg
            if not accepted:
                decision_by_limb[limb_name]["winner"] = None
                decision_by_limb[limb_name]["reason"] = "angle_limit"
        if not accepted:
            continue
        for name in joints:
