import numpy as np
from src.pipelines.fusion.config import TORSO_PART_IDS
from src.pipelines.fusion.config import ROTATION_PARENT_JOINTS
from src.pipelines.fusion.config import NON_REPLACEABLE_ANCHORS
from src.pipelines.fusion.config import HARMONIC_EPSILON
...
def compute_harmonic_precision(
    ...
    def calc_H(P):
        ...
        for name in joint_names:
            ...
            b = (1.0 - beta) * p + beta * (sum(nb) / len(nb)) if nb else p
