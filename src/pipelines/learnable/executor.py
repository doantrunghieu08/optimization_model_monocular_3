...
from src.core.compat import load_joblib_compat

LEARNABLE_VENDOR_ROOT = Path(__file__).resolve().parent / "backend"
...
LEARNABLE_EXTRA_FILE_PREFIX = "learnable_extra_frame_"
...
def _clean_learnable_output(output_dir: Path, file_prefix: str) -> None:
...
def _slice_person(person: dict, start: int, frame_count: int) -> dict:
    ...
    if total_frames == 0:
        raise ValueError("WHAM person pose data is empty")
    start = max(0, min(start, total_frames - 1))
    valid_count = min(frame_count, total_frames - start)
    out = {}
    for key, value in person.items():
        if isinstance(value, np.ndarray) and value.ndim > 0 and value.shape[0] == total_frames:
            out[key] = value[start:start + valid_count].copy()
        else:
            out[key] = value
    return out
...
def _stage_metadata_key(stage_label: str) -> str:
    if stage_label == "LearnableExtra":
        return "learnable_extra"
    return "learnable_smplify"


def _resolve_branch_cfg(config: dict, section_key: str) -> dict:
    branch_cfg = config.get(section_key)
    if not isinstance(branch_cfg, dict):
        raise ValueError(f"Missing config section: {section_key}")
    return branch_cfg


def run_learnable_smplify(config: dict) -> None:
    _run_learnable_smplify_stage(
        config=config,
        input_dir_key="fused_output_dir",
        input_file_prefix="fused_data_",
        output_dir_key="learnable_output_dir",
        output_file_prefix=LEARNABLE_FILE_PREFIX,
        stage_label="Learnable",
        branch_cfg_key="learnable",
    )


def _run_learnable_smplify_stage(
    config: dict,
    input_dir_key: str,
    ...
    learnable_cfg = copy.deepcopy(_resolve_branch_cfg(config, branch_cfg_key))

    if not learnable_cfg["enabled"]:
        print(f"[{stage_label}] Disabled by config: {branch_cfg_key}.enabled=false")
        return

    input_dir = Path(paths[input_dir_key])
    output_dir = Path(paths[output_dir_key])
    ...
    print(f"[{stage_label}] Learnable-SMPLify from {input_dir_key}")
    judgement_results = load_stage_results(input_dir, input_file_prefix, stage_label)
    ...
    wham_frames = min(len(wham_data["camera1"]["pose"]), len(wham_data["camera2"]["pose"]))
    frame_count = min(frame_count, wham_frames)
    judgement_results = judgement_results[:frame_count]
    ...
    print(f"[{stage_label}] Done. Output: {output_dir}")


def run_learnable_smplify_extra(config: dict) -> None:
    _run_learnable_smplify_stage(
        config=config,
        input_dir_key="pose_output_dir",
        input_file_prefix="pose_data_",
        output_dir_key="learnable_extra_output_dir",
        output_file_prefix=LEARNABLE_EXTRA_FILE_PREFIX,
        stage_label="LearnableExtra",
        branch_cfg_key="learnable_extra",
    )
